...
function calcScore(game, cells) {
  const BONUSES = {};
  [[0,0],[0,7],[0,14],[7,0],[7,14],[14,0],[14,7],[14,14]].forEach(([r,c])=>BONUSES[r+'_'+c]='TW');
  [[1,1],[2,2],[3,3],[4,4],[10,10],[11,11],[12,12],[13,13],[1,13],[2,12],[3,11],[4,10],[10,4],[11,3],[12,2],[13,1],[7,7]].forEach(([r,c])=>BONUSES[r+'_'+c]='DW');
  [[1,5],[1,9],[5,1],[5,5],[5,9],[5,13],[9,1],[9,5],[9,9],[9,13],[13,5],[13,9]].forEach(([r,c])=>BONUSES[r+'_'+c]='TL');
  [[0,3],[0,11],[2,6],[2,8],[3,0],[3,7],[3,14],[6,2],[6,6],[6,8],[6,12],[7,3],[7,11],[8,2],[8,6],[8,8],[8,12],[11,0],[11,7],[11,14],[12,6],[12,8],[14,3],[14,11]].forEach(([r,c])=>BONUSES[r+'_'+c]='DL');
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'create_room') {
      ...
      const cells = Object.keys(game.placed).filter(k=>!k.startsWith('rack_')).map(k=>{const[r,c]=k.split('_').map(Number);return{r,c,l:game.placed[k]};});
      ...
      let word='',allCells=[];
      if(isHoriz){for(let c=minC;c<=maxC;c++){const l=game.board[minR][c]?.l||game.placed[minR+'_'+c];if(!l){sendTo(ws,{type:'error',message:'Espace vide.'});return;}word+=l;allCells.push({r:minR,c,l});}}
      else{for(let r=minR;r<=maxR;r++){const l=game.board[r][minC]?.l||game.placed[r+'_'+minC];if(!l){sendTo(ws,{type:'error',message:'Espace vide.'});return;}word+=l;allCells.push({r,c:minC,l});}}
      if(game.firstMove&&!game.placed['7_7']){sendTo(ws,{type:'error',message:'Le premier mot doit passer par ★.'});return;}
      if(!game.firstMove){const adj=cells.some(({r,c})=>[[r-1,c],[r+1,c],[r,c-1],[r,c+1]].some(([nr,nc])=>nr>=0&&nr<15&&nc>=0&&nc<15&&game.board[nr][nc]));if(!adj){sendTo(ws,{type:'error',message:'Le mot doit être connecté.'});return;}}
